...
#include "malgo.h"
#include "callbacks.h"
static void setGoDeviceCallbacks(ma_device_config* config, ma_bool8 data, ma_bool8 stop) {
	if(data) {
...
func (config *DeviceConfig) toCRepr() (C.ma_device_config, pointerList) {
	...
	C.setGoDeviceCallbacks(&cConfig, boolToInt8(config.DataCallback != nil), boolToInt8(config.StopCallback != nil))
